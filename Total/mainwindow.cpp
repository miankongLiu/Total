...
{       QDialog *d=new QDialog(this);
        ...
        dlg->setFileMode(QFileDialog::AnyFile);
        dlg->setViewMode(QFileDialog::Detail);
        //dlg->setNameFilter(QString(tr("*.mod")));
        QString root;
        if(dlg->exec()==QFileDialog::Accepted)
        {
        ...
        //存储x与误差值
        //区分两种格式
        //2020-1-9 mod、txt格式--平均值、曲线
       if(root.contains(".mod")){
                QVector<double> pointTxt;
                if(root!=nullptr){
                    QFile file(root);
                    if(file.open((QIODevice::ReadOnly|QIODevice::Text))){
                        ...
                        while(!file.atEnd()){
                            ...
                            QString str(line);
                            if(!str.contains(" ")&&str[0].isLetter()==false){
               ...
               int pointNum=pointTxt.size();
               for(int i=0;i<pointNum;i++){
                   if(i<pointNum/2){
                       x_Test<<pointTxt.at(i);
                   }else{
                       z_Test<<pointTxt.at(i);
                   }
               ...
               for(int i=0;i<x_Test.size();i++){
                   if(x_Test[i]==0.0) pos=i;
               }
               //x_Test.erase(x_Test.begin(),x_Test.begin() + (pointNum/4));
               //z_Test.erase(z_Test.begin(),z_Test.begin() + (pointNum/4));
               x_Test.erase(x_Test.begin(),x_Test.begin()+pos);
               z_Test.erase(z_Test.begin(),z_Test.begin()+pos);
            }
        else if(root.contains("txt")){
                if(root!=nullptr){
                    QVector<double> x_,z_;
                            QFile file(root);
                                if (file.open(QIODevice::ReadOnly | QIODevice::Text))
                                {
                                    while (!file.atEnd())
...
*/
         ...
         QMessageBox::information(this,"提示","格式错误！",QMessageBox::Yes);
         return;
       }
       QDateTime datetime;
       QString  fileName;
       QString timestr=datetime.currentDateTime().toString("yyyy-MM-dd-HH-mm");
       fileName = dir_str+ "/"+timestr +"转换误差"+".xlsx";//假设指定文件夹路径为D盘根目录
       fileName.replace("/","\\");
...
{
    //要打开的程序的路径
        QString path(".\\fit\\fitMachine.exe");
        //新建一个进程
